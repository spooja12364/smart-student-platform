...
class _RegisterPageState extends State<RegisterPage> {
  ...
  Future<void> sendOTP() async {
    ...
    try {
      ...
      if (emailSnapshot.snapshot.value != null || firestoreSnapshot.docs.isNotEmpty) {
      ...
      _setError('Unable to send OTP. Please try again. Use OTP: $generatedOtp');
    } finally {
      if (mounted) {
  ...
  Future<void> verifyOTP() async {
    ...
    try {
      final snapshot = await FirebaseDatabase.instance.ref('otpCodes/$_otpKey').once();
      if (snapshot.snapshot.value == null) {
        _setError('OTP not found. Please generate again.');
        return;
      }
...
Path('lib/register.dart').write_text(content)
print('register.dart rewritten successfully')
